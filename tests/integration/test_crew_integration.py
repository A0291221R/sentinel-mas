...
def mock_all_dependencies():
    ...
    with patch('sentinel_mas.agents.crew_agents.ChatOpenAI') as mock_chat_openai, \
        ...
        from sentinel_mas.agents.crew import router_condition, parse_time_node
...
class TestCrewIntegration:
    ...
    def test_router_condition_valid_routes(self):
        """Test YOUR router_condition function with valid route decisions"""
        test_cases = [
            ('{"route": "SOP", "confidence": 0.9, "reason": "procedure question"}', "SOP"),
            ('{"route": "EVENTS", "confidence": 0.8, "reason": "event query"}', "EVENTS"),
            ('{"route": "TRACKING", "confidence": 0.7, "reason": "tracking request"}', "TRACKING"),
        ]
    ...
    def test_router_condition_invalid_cases(self):
    ...
    def test_parse_time_node_success(self, mock_resolve_time):
        """Test YOUR parse_time_node function with successful parsing"""
        # Mock the time resolution
        mock_resolve_time.return_value = (1704067200000, 1704070800000, "last 30 minutes")
    ...
    def test_parse_time_node_already_parsed(self):
    ...
    @patch('sentinel_mas.agents.crew.resolve_time_window')
    def test_parse_time_node_failure(self, mock_resolve_time):
