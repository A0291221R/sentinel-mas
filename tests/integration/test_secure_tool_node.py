...
class TestSecureToolNode:
    ...
    @pytest.fixture
    def secure_tool_node(self, mock_tools: List[Mock]) -> SecureToolNode:
        """Create SecureToolNode instance for testing"""
        return SecureToolNode(
            route="TRACKING", tools=mock_tools, agent_name="test_agent"
        )

    def test_initialization(self, mock_tools: List[Mock]) -> None:
    ...
    def test_initialization_missing_tool_name(self) -> None:
    ...
    def test_get_route_from_state(self, mock_tools: List[Mock]) -> None:
        """Test route extraction from state"""
        node = SecureToolNode(route="DEFAULT", tools=mock_tools, route_from_state=True)
    ...
    def test_last_ai_message_extraction(
        self, secure_tool_node: SecureToolNode, sample_state: Dict[str, Any]
    ) -> None:
    ...
    def test_last_ai_no_tool_calls(
        self, secure_tool_node: SecureToolNode, sample_state: Dict[str, Any]
    ) -> None:
    ...
    def test_successful_tool_execution(
        ...
        mock_secure_execute: MagicMock,
        mock_graph_scope: MagicMock,
        mock_context_scope: MagicMock,
        secure_tool_node: SecureToolNode,
        sample_state: Dict[str, Any],
        mock_tools: List[Mock],
    ) -> None:
    ...
    def test_multiple_tool_executions(
        ...
        def tool_registry_side_effect(tool_name: str) -> Optional[Mock]:
        ...
        def secure_execute_side_effect(
            tool_name: str, tool_fn: Mock, tool_args: Dict[str, Any]
        ) -> Dict[str, Any]:
            if tool_name == "test_tool_1":
    ...
    def test_mixed_success_and_failure(
        ...
        # Create state with multiple tool calls
        tool_calls = [
            {"name": "test_tool_1", "args": {}, "id": "call_1", "type": "tool_call"},
            {"name": "test_tool_2", "args": {}, "id": "call_2", "type": "tool_call"},
        ]
        ai_message = AIMessage(content="", tool_calls=tool_calls)
        sample_state["messages"] = [ai_message]
    ...
    def test_state_overrides(self, secure_tool_node: SecureToolNode) -> None:
