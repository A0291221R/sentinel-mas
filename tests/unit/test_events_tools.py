# sentinel_mas/tools/tests/test_events_tools.py (fix parameter access)
import pytest
from unittest.mock import Mock, MagicMock
from sentinel_mas.tools.events_tools import who_entered_zone, list_anomaly_event, _clamp_limit
...
class TestEventsTools:
    ...
    def test_who_entered_zone_success(self, mock_db_connection):
        """Test who_entered_zone with successful database response"""
        mock_connect, mock_conn, mock_cursor = mock_db_connection
        
        # Mock database response
        mock_cursor.description = [
            ('track_id',), ('resolved_id',), ('location_id',), 
            ('appear_ms',), ('disappear_ms',), ('cam_id',), ('appear_at_sgt',)
        ]
        mock_cursor.fetchall.return_value = [
            ('track_123', 'person_456', 'loc_789', 1609459200000, 1609459260000, 'cam_001', '2021-01-01 12:00:00')
        ]
    ...
    def test_who_entered_zone_no_camera_filter(self, mock_db_connection):
    ...
    def test_list_anomaly_event_success(self, mock_db_connection):
        """Test list_anomaly_event with successful database response"""
        mock_connect, mock_conn, mock_cursor = mock_db_connection
        
        mock_cursor.description = [
            ('ts_ms',), ('location_id',), ('cam_id',), ('incident',), 
            ('phase',), ('confidence',), ('episode',), ('ad_event_id',),
            ('end_ms',), ('duration_ms',), ('ts_at_sgt',)
        ]
        mock_cursor.fetchall.return_value = [
            (1609459200000, 'loc_789', 'cam_001', 'intrusion', 'active', 0.95, 'ep_123', 'event_456', 1609459260000, 60000, '2021-01-01 12:00:00')
        ]
    ...
    def test_list_anomaly_event_no_filters(self, mock_db_connection):
