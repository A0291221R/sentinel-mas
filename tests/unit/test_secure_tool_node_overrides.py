...
def test_route_from_state_overrides_ctor_route(monkeypatch) -> None:
...
def test_state_overrides_freeze_time_window_and_extra_keys(monkeypatch) -> None:
    ...
    ai = _ai_with_tools(
        {"name": "echo", "args": {"start_ms": 1, "end_ms": 2, "location_id": "X"}}
    )
    state = {
        "messages": [ai],
        "start_ms": 1111,
