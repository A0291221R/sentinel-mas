...
class TestLoader:
    def test_load_tools_integration(self):
        ...
        expected_tools = ['get_sop', 'search_sop', 'list_anomaly_event', 'who_entered_zone']
    ...
    def test_load_langchain_decorated_tools_structure(self):
    ...
    def test_tool_registry_consistency(self):
        ...
        assert set(TOOL_REGISTRY.keys()) == set(loaded_tools.keys()), "Tool registry keys should match"
        
        # Check that tool names match
        for tool_name in TOOL_REGISTRY:
            assert TOOL_REGISTRY[tool_name].name == loaded_tools[tool_name].name, f"Tool name mismatch for {tool_name}"
...
class TestToolModules:
    def test_events_tools_import(self):
    ...
    def test_tracking_tools_import(self):
        ...
        try:
            ...
            assert hasattr(get_person_insight, 'func') and callable(get_person_insight.func)
        except ImportError as e:
