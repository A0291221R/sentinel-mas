...
class TestConfigurationRealTools:
    ...
    def test_router_agent_config_loading(self) -> None:
        """Test loading router agent config (no tools)"""
        with tempfile.TemporaryDirectory() as temp_dir:
            test_config = {
                "name": "router_agent",
                "llm": {"model": "gpt-4o-mini", "temperature": 0.0, "max_tokens": 64},
    ...
    def test_faq_agent_config_loading(self) -> None:
        """Test loading FAQ agent config with SOP tools"""
        with tempfile.TemporaryDirectory() as temp_dir:
            test_config = {
                "name": "faq_agent",
                "llm": {"model": "gpt-4o-mini", "temperature": 0.0, "max_tokens": 350},
            ...
            mock_tool_registry = {"get_sop": MagicMock(), "search_sop": MagicMock()}
    ...
    def test_events_agent_config_loading(self) -> None:
        """Test loading events agent config with event tools"""
        with tempfile.TemporaryDirectory() as temp_dir:
            test_config = {
                "name": "events_agent",
                "llm": {"model": "gpt-4o-mini", "temperature": 0.0, "max_tokens": 1300},
    ...
    def test_tracking_agent_config_loading(self) -> None:
        """Test loading tracking agent config with tracking tools"""
        with tempfile.TemporaryDirectory() as temp_dir:
            test_config = {
                "name": "tracking_agent",
                "llm": {"model": "gpt-4o-mini", "temperature": 0.0, "max_tokens": 300},
                "tools": [
                    "send_track",
                    "send_cancel",
                    "get_track_status",
