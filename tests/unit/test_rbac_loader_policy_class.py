# tests/unit/test_rbac_loader_policy_class.py
import pytest
...
def test_is_allowed_false_when_role_missing(tmp_path):
...
def test_route_denied_and_tool_denied(tmp_path):
    p = _write_policy(tmp_path, """
roles:
  operator:
    routes_allowed: [SOP]
...
def test_get_allowed_tools_and_assert_allowed(tmp_path):
    p = _write_policy(tmp_path, """
roles:
  analyst:
...
""")
    ...
    # No route filter
    assert set(policy.get_allowed_tools("analyst")) == {"list_anomaly_event", "send_track"}

    # Route filter OK
    assert set(policy.get_allowed_tools("analyst", "EVENTS")) == {"list_anomaly_event", "send_track"}
...
def test_describe_and_get_roles(tmp_path):
    p = _write_policy(tmp_path, """
roles:
  viewer:
    routes_allowed: [SOP]
