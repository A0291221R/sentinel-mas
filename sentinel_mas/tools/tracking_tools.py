            ...
            with httpx.Client(timeout=timeout, headers=_headers()) as client:
                ...
                if resp.status_code >= 400:
                    try:
                        ...
                        detail = resp.text
                    return {"ok": False, "status_code": resp.status_code, "error": detail, "endpoint": path}
                return {"ok": True, "status_code": resp.status_code, "data": resp.json(), "endpoint": path}
        except (httpx.ConnectError, httpx.ReadTimeout) as e:
            if attempt < MAX_RETRIES:
                time.sleep(RETRY_BACKOFF * (attempt + 1))
                continue
            return {"ok": False, "status_code": 599, "error": f"{type(e).__name__}: {e}", "endpoint": path}

@tool
def send_track(resolved_id: str) -> Dict[str, Any]:
    """Activate server-side tracking for a person/identity by resolved_id."""
    if not resolved_id:
        return {"ok": False, "status_code": 400, "error": "resolved_id is required", "endpoint": "/person/track"}
...
def send_cancel(resolved_id: str) -> Dict[str, Any]:
    ...
    if not resolved_id:
        return {"ok": False, "status_code": 400, "error": "resolved_id is required", "endpoint": "/person/untrack"}
...
def get_track_status(resolved_id: str) -> Dict[str, Any]:
    ...
    if not resolved_id:
        return {"ok": False, "status_code": 400, "error": "resolved_id is required", "endpoint": "/person/{id}/tracking"}
...
def get_person_insight(resolved_id: str) -> Dict[str, Any]:
    ...
    if not resolved_id:
        return {"ok": False, "status_code": 400, "error": "resolved_id is required", "endpoint": "/insight/{id}"}
